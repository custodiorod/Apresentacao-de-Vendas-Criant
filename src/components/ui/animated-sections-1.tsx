...
const AnimatedSections: React.FC<AnimatedSectionsProps> = ({
  ...
  const gotoSection = useCallback((index: number, direction: number) => {
    ...
    const headings = headingRefs.current as Element[];
    ...
    gsap.set(headings[index], { opacity: 0, yPercent: 100 });
    tl.to(
      headings[index],
      {
        opacity: 1,
    ...
    if (counterCurrentRef.current && counterNextRef.current) {
      ...
      gsap.set(counterCurrentRef.current, { opacity: 1, yPercent: 0 });
      gsap.set(counterNextRef.current, { opacity: 1, yPercent: 100 * dFactor });
        ...
        >
          <div className="outer w-full h-full overflow-hidden" ref={(el) => { if (el) outerRefs.current[i] = el; }}>
            <div className="inner w-full h-full overflow-hidden" ref={(el) => { if (el) innerRefs.current[i] = el; }}>
              <div
                className="bg flex items-center justify-center absolute top-0 h-full w-full bg-no-repeat bg-contain bg-center"
